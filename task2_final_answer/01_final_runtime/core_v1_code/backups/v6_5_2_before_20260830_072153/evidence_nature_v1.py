...
def _risk_control_subject_match(exact_quote: str) -> bool:
    """Require a contamination/infestation/hygiene/pest-control subject."""
    evid = _norm(exact_quote)
    return bool(re.search(
        ...
        r"mould|mold|pathogen|biosecurity|(?:bait )?stations?|traps?)\b",
        evid,
    ))


def _requirement_is_holistic_risk_condition(requirement: dict) -> bool:
    text = _norm(str(requirement.get("proposition_to_establish", "")))
    return bool(re.search(
        r"\b(minimi[sz](?:e|ed|ing)?|acceptable level|risk of)\b", text
    ) and re.search(
        r"\b(contamination|infestation|pest harbourage|pests?|contaminants?)\b", text
    ))


def _explicit_holistic_design_support(text: str) -> bool:
    """A local physical feature is not enough to prove a whole-establishment design claim."""
    t = _norm(text)
    subject = bool(re.search(r"\b(establishment|facility|premises|processing shed|building)\b", t))
    design = bool(re.search(r"\b(design(?:ed)?|construct(?:ed|ion)?|purpose[- ]built)\b", t))
    risk = bool(re.search(
        r"\b(minimi[sz](?:e|ed|ing)?|contamination|infestation|pest harbourage|pest exclusion)\b", t
    ))
    return subject and design and risk


def _explicit_positive_risk_outcome(text: str) -> bool:
    t = _norm(text)
    return bool(re.search(
        r"\b(within acceptable (?:range|level)|below threshold|pest[- ]free|no active infestation|"
        r"no rodent activity|no pest activity|free of (?:pests?|contamination|infestation)|"
        r"negative for live insects|no evidence of (?:bird|pest|rodent|insect) entry|"
        r"no evidence of access to grain-contact areas|current pest status[^.]{0,100}acceptable)\b",
        t,
    ))


def _explicit_adverse_risk_outcome(text: str) -> bool:
    t = _norm(text)
    return bool(re.search(
        r"\b(active infestation|pest activity (?:was )?(?:found|identified|detected)|"
        r"repeated (?:rodent|pest|insect) activity|above threshold|contamination (?:found|detected|present)|"
        r"delayed .*follow[- ]?up|control failure|not controlled)\b",
        t,
    ))
...
def assess_alignment_compatibility(
    ...
    *,
    ...
    if evid["requires_subspan_fact_split"]:
        ...
        if model_relation == "SUPPORT":
            # V6.4 precision rule: an acceptable-risk proposition needs an
            # explicit outcome-level statement (acceptable/below threshold/
            # pest-free/negative monitoring), not merely generic cleanliness.
            if (
                mode not in {MODALITY_REQUIRED, MODALITY_PLANNED}
                and _explicit_positive_risk_outcome(semantic_text)
            ):
                decision = DIRECT
                reason = "RISK_CONTROL_STATE_SUPPORTED_BY_EXPLICIT_ACCEPTABLE_RISK_OUTCOME"
            elif natures & {
                ...
                decision = CORROBORATIVE
                reason = "CURRENT_HYGIENE_OR_REVIEW_STATE_IS_NOT_PROPOSITION_COMPLETE_RISK_SUPPORT"
            elif natures & {
                NATURE_PROCEDURE_STATEMENT,
            ...
            elif _explicit_adverse_risk_outcome(semantic_text):
                decision = DIRECT
                reason = "RISK_CONTROL_STATE_ATTACKED_BY_EXPLICIT_UNACCEPTABLE_RISK_OUTCOME"
            elif natures & {NATURE_ADVERSE_OPERATIONAL_FINDING, NATURE_CURRENT_DEFECT}:
                decision = CORROBORATIVE
                reason = "LOCAL_DEFECT_OR_ADVERSE_FINDING_DOES_NOT_BY_ITSELF_PROVE_UNACCEPTABLE_RISK"
            elif NATURE_DOCUMENTATION_GAP in natures:
                decision = CORROBORATIVE
        ...
        if model_relation == "SUPPORT":
            # V6.4 precision rule: a local positive feature (steel frame, one
            # sealed opening, one tamper-resistant station, etc.) is not a
            # proposition-complete proof that the whole establishment was
            # designed/constructed to minimise contamination/infestation.
            # DIRECT is reserved for an explicit holistic design claim or for
            # a deterministic structural aggregate emitted upstream.
            if NATURE_PHYSICAL_FEATURE in natures and _explicit_holistic_design_support(semantic_text):
                decision = DIRECT
                reason = "DESIGN_SUPPORTED_BY_EXPLICIT_HOLISTIC_DESIGN_STATEMENT"
            elif NATURE_PHYSICAL_FEATURE in natures:
                decision = CORROBORATIVE
                reason = "LOCAL_DESIGN_FEATURE_DOES_NOT_PROVE_HOLISTIC_DESIGN_REQUIREMENT"
            elif natures & {
                NATURE_PROCEDURE_STATEMENT,
                NATURE_PLAN_STATEMENT,
                NATURE_CURRENT_CONDITION,
        ...
        if model_relation == "SUPPORT":
            ...
            # separated into facts.
                ...
                # V6.4: for holistic contamination/infestation maintenance
                # requirements, generic operational condition is not enough.
                # The current-state fact must actually bear on hygiene/pest/
                # contamination control; structural aggregates may combine the
                # hygiene and pest-condition sections into proposition-complete
                # support.
                if (
                    _requirement_is_holistic_risk_condition(requirement)
                    and not _risk_control_subject_match(semantic_text)
                ):
                    decision = CORROBORATIVE
                    reason = "GENERIC_CURRENT_STATE_DOES_NOT_PROVE_HOLISTIC_RISK_CONDITION"
                else:
                    decision = DIRECT
            ...
            if natures & {
                ...
                if (
                    ...
                    decision = CORROBORATIVE
                    reason = "UNRELATED_MAINTENANCE_DEFECT_DOES_NOT_PROVE_HOLISTIC_RISK_FAILURE"
                else:
                    decision = DIRECT
