...
"""Versioned V6.4 production runner.

Keeps the V1 manifest/parsing/retrieval/LLM path, then adds V6.4 deterministic
structural witnesses before Layer-7 proof.  V6.4 extends the structural path to
...
def run_initial_requirement_reasoning_v6_4(*, case, cp_id, chunks, plan, task_dir, retrieval_top_k):
    rr = _ORIGINAL(
        case=case,
        cp_id=cp_id,
        chunks=chunks,
    ...
    enriched["production_semantic_version"] = "V6.4_CONFLICT_PRECISION_STRUCTURAL"
...
base.run_initial_requirement_reasoning = run_initial_requirement_reasoning_v6_4
for _name in (
    "structured_witness_v6_3.py",
    "production_runner_v6_4.py",
):
