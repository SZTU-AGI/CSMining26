...
def classify_capability(contract: dict):
    blockers = []
    ...
    if set(collect_ops(sat)) - {"ATOM"}:
        blockers.append("COMPOSITE_SATISFACTION_REQUIRES_AST_OR_GRAPH_EVALUATOR")
    if len(contract.get("atoms") or []) != 1:
        blockers.append("MULTI_ATOM_CONTRACT_REQUIRES_GENERAL_GRAPH_PROJECTION")
    return (
...
def run_self_tests():
    ...
    print("contract_shape_inventory_v1_2 self-tests: PASS")
...
def main():
    ...
    for path in files:
        ...
        status, blockers = classify_capability(contract)
    ...
    result = {
        "schema": "freca-core-contract-shape-inventory-v1.2",
    ...
    print("=" * 78)
    print("FRECA CANONICAL CONTRACT SHAPE INVENTORY V1.2")
    print("=" * 78)
