...
def deepseek_json(
    *,
    ...
    # Temperature is pinned on every call, reasoning included.
    #
    # It used to be set only on the non-thinking branch, which left contract
    # compilation sampling at the provider default. The competition rules
    # disqualify a submission that cannot be reproduced from the declared
    # prompt and model, so an unpinned decoding parameter on any call is a
    # qualification risk, not merely a source of variance. Contracts are
    # written to disk and hashed into each task fingerprint, so a run is
    # reproducible given the contracts; rebuilding them from scratch was not.
    #
    # FRECA_UNPIN_THINKING_TEMPERATURE=1 restores the old behaviour without a
    # code change, in case a provider rejects temperature alongside reasoning.
    if not (
        thinking
        and os.environ.get(
            "FRECA_UNPIN_THINKING_TEMPERATURE"
        ) == "1"
    ):
        payload[
            "temperature"
