...
def classify_temporal_requirement(
    ...
    for source, value in candidates:
        ...
        elif value is False or normalized in {
            "FALSE",
            "NOT_REQUIRED",
            "TEMPORAL_NOT_REQUIRED",
        }:
...
def apply_gate_assessments(
    *,
    ...
    for row in patched.get("alignments", []):
        ...
        if temporal and temporal["state"] == "TEMPORAL_NOT_REQUIRED":
            row["temporal_requirement_classification"] = copy.deepcopy(temporal)
            row["temporal_relation"] = "NOT_REQUIRED"
        elif temporal and temporal["state"] == "TEMPORAL_REQUIRED":
            row["temporal_requirement_classification"] = copy.deepcopy(temporal)
            if not row.get("temporal_assessment") and not row.get("temporal_relation"):
                row["temporal_relation"] = "UNRESOLVED"
        elif temporal:
            row["temporal_requirement_classification"] = copy.deepcopy(temporal)
            if not row.get("temporal_assessment") and not row.get("temporal_relation"):
                row["temporal_relation"] = "UNRESOLVED"
...
def evaluate_proof_standard_bundle(
    ...
    bundle = {
        ...
        "proof_policy_version": "DIRECTIONAL_PROOF_GATE_V2_1",
