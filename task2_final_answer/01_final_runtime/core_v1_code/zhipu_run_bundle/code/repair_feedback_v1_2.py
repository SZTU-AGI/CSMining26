...
def alignment_artifact_id(row: dict) -> str:
    """Return the source/fact identity without its requirement relation."""
    return str(
...
def alignment_id(row: dict) -> str:
    """Return the semantic identity of an evidence-to-requirement alignment.

    One grounded fact may be evaluated against multiple requirements.  Using
    the fact identity alone silently collapsed those distinct semantic rows
    during repair merge.
    """
    artifact_id = alignment_artifact_id(row)
    ...
    for execution in round_bundle.get("action_executions", []):
        for row in execution.get("new_alignments", []):
            ...
            if aid not in by_id:
                ...
                by_id[aid] = merged_row
                rows.append(merged_row)
                continue

            # The same semantic alignment may be reached through both the
            # support and attack retrieval paths. Preserve all registered
            # need bindings instead of silently keeping the first path only.
            existing = by_id[aid]
            existing["retrieval_need_ids"] = sorted(
                set(existing.get("retrieval_need_ids", []) or [])
                | set(row.get("retrieval_need_ids", []) or [])
            )
...
def build_alignment_lookup(rows: list[dict]) -> dict[str, dict]:
    ...
    for row in rows:
        ...
        artifact_id = alignment_artifact_id(row)
        if artifact_id:
            # Backward-compatible fallback for legacy assessments that do not
            # carry requirement_id. Do not let later cross-requirement rows
            # overwrite the deterministic first match.
            lookup.setdefault(artifact_id, row)
        ...
        if fc:
            requirement_id = str(row.get("requirement_id") or "").strip()
            if requirement_id:
                lookup[requirement_id + "::" + str(fc)] = row
            lookup.setdefault(str(fc), row)
...
def merge_round_into_requirement_result(
    ...
    for assessment in round_temporal_assessments(round_bundle):
        ...
        target_key = alignment_id(
            {
                "requirement_id": assessment.get("requirement_id"),
                "alignment_evidence_id": target,
            }
        )
        row = lookup.get(target_key) or lookup.get(target)
...
def duplicate_alignment_semantic_violations(
    round_bundle: dict,
) -> list[str]:
    """Reject inconsistent model judgments for one semantic alignment.

    Duplicate rows reached through different retrieval needs are safe to merge
    only when their semantic and argument-visible fields agree.
    """
    seen = {}
    violations = []
    ...
    for execution in round_bundle.get("action_executions", []):
        for row in execution.get("new_alignments", []):
            ...
            if norm_ws(prior.get("exact_quote")) != norm_ws(
                row.get("exact_quote")
            ):
                changed.append("exact_quote")

            if changed:
                violations.append(
                    aid
                    + ":DUPLICATE_SEMANTIC_JUDGMENT_CONFLICT:"
                    + ",".join(sorted(changed))
                )
...
def affected_requirement_ids(
    *,
    ...
    for assessment in (
        ...
        old = old_lookup.get(target_key) or old_lookup.get(target)
...
def evaluate_hard_gates(
    *,
    ...
    gate_violations = {
        ...
        "duplicate_semantic_judgment":
            duplicate_alignment_semantic_violations(
                round_bundle
            ),
...
def cost_vector(round_bundle: dict) -> dict:
    """Report persisted experimental cost without changing comparison semantics."""

    cost = round_bundle.get("cost_telemetry")

    if not isinstance(cost, dict):
        # Backward-compatible aggregation from action artifacts.
        action_costs = [
            row.get("cost_telemetry")
            for row in round_bundle.get("action_executions", [])
            if isinstance(row.get("cost_telemetry"), dict)
        ]

        persisted = [
            row
            for row in action_costs
        ...
        if persisted:
            cost = {
                ...
                "request_attempt_count":
                    sum(int(x.get("request_attempt_count", 0) or 0) for x in persisted),
                "successful_call_count":
                    sum(int(x.get("successful_call_count", 0) or 0) for x in persisted),
                "failed_call_count":
                    sum(int(x.get("failed_call_count", 0) or 0) for x in persisted),
                "prompt_tokens":
                    sum(int(x.get("prompt_tokens", 0) or 0) for x in persisted),
                "completion_tokens":
                    sum(int(x.get("completion_tokens", 0) or 0) for x in persisted),
                "total_tokens":
                    sum(int(x.get("total_tokens", 0) or 0) for x in persisted),
                "wall_time_ms":
                    sum(int(x.get("wall_time_ms", 0) or 0) for x in persisted),
            }

    if isinstance(cost, dict) and cost.get("status") == "PERSISTED":
        return {
            "executed_action_count":
                len(round_bundle.get("action_executions", [])),
            ...
            "alignment_target_parent_count":
                sum(
                    len(
                        execution.get(
                            "selected_parent_ids",
                            execution.get("target_artifact_ids", []),
                        )
                    )
                    for execution in round_bundle.get("action_executions", [])
            ...
            "interpretation":
                (
                    "Cost telemetry was captured transparently at the DeepSeek "
                    "HTTP boundary. It is reported as a comparison dimension "
                    "but does not alter Hard Gates, substantive_change, or "
                    "lexicographic ranking priorities."
                ),
        }
    ...
    return {
        ...
        "interpretation":
            (
                "No persisted cost telemetry is present. Cost is not inferred "
                "from console logs."
            ),
    }
...
def build_evaluation_diff(
    *,
    ...
    diff = {
        "schema":
            "freca-core-evaluation-diff-v1.2",
...
def run_self_tests() -> None:
    ...
    # Alignment identity is evidence-to-requirement, not evidence alone. The
    # same requirement/evidence reached through two directional needs is one
    # semantic row with both need bindings; another requirement is distinct.
    collision_bundle = {
        "action_executions": [
            {
                "need_id": "ER1.attack",
                "new_alignments": [
                    {
                        "requirement_id": "ER1",
                        "alignment_evidence_id": "shared#1",
                ...
                "new_alignments": [
                    {
                        "requirement_id": "ER1",
                        "alignment_evidence_id": "shared#1",
                        "retrieval_need_ids": ["ER1.support"],
                        "relation": "SUPPORT",
                        "argument_admission_channel": "DIRECT",
                        "argument_truth_bearing": True,
                    }
                ...
                "new_alignments": [
                    {
                        "requirement_id": "ER2",
                        "alignment_evidence_id": "shared#1",
                        "retrieval_need_ids": ["ER2.support"],
                        "relation": "SUPPORT",
                        "argument_admission_channel": "DIRECT",
                        "argument_truth_bearing": True,
                    }
    ...
    collision_rows = round_new_alignments(collision_bundle)
    assert len(collision_rows) == 2
    er1_row = next(
        row
        for row in collision_rows
        if row["requirement_id"] == "ER1"
    )
    assert er1_row["retrieval_need_ids"] == [
        "ER1.attack",
        "ER1.support",
    ]

    collision_signals = verified_signal_gain(collision_bundle)
    assert collision_signals["truth_bearing_alignment_count"] == 2
    assert collision_signals["goal_aligned_verified_signal_count"] == 2
    assert collision_signals["off_goal_verified_signal_count"] == 0

    conflicting_duplicate = copy.deepcopy(collision_bundle)
    conflicting_duplicate["action_executions"][1][
        "new_alignments"
    ][0]["relation"] = "ATTACK"
    assert duplicate_alignment_semantic_violations(
    ...
    print("repair_feedback_v1_2 self-tests: PASS")
    ...
    print("  semantic alignment identity preserves cross-requirement rows")
    print("  duplicate directional paths merge retrieval-need bindings")
    print("  inconsistent duplicate judgments fail a hard gate")
...
def main() -> None:
    ...
    print("=" * 72)
    print("FRECA REPAIR FEEDBACK / EVALUATION DIFF V1.2")
    print("=" * 72)
