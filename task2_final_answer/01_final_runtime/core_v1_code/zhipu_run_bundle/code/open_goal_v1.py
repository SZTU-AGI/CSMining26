...
def run_self_tests() -> None:
    ...
    # Untyped temporal applicability is a terminal input limitation, not an
    # executable missing-assessment goal.
    terminal_temporal_goals = build_resolution_goals(
        case_uid="case-x",
        cp_id="CPX",
        interpretation_id="interp-x",
        requirement=rr["evidence_requirement_plan"]["requirements"][0],
        trace=rr["retrieval_traces"][0],
        requirement_proof={
            "requirement_id": "ER1",
            "support_proof": {
                "basis_artifact_ids": ["a1"],
                "failure_codes": ["TEMPORAL_REQUIREMENT_UNRESOLVED"],
            },
            "attack_proof": {
                "basis_artifact_ids": [],
                "failure_codes": [],
    ...
    assert not any(
        goal["goal_type"] == "RESOLVE_TIME"
        for goal in terminal_temporal_goals
    )
