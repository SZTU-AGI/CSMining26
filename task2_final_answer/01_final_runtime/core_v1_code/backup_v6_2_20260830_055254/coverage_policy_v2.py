...
def evaluate_coverage_bundle(
    ...
    *,
    ...
    for need_id, trace in sorted(traces.items()):
        ...
        if purpose in {"POSITIVE_EXISTENCE_PROOF", "EXPLICIT_ADVERSE_PROOF"}:
            # V6.1 directional-existence semantics:
            # a grounded direct basis may establish its own direction once the
            # registered discovery procedure for that direction has completed.
            # Exhaustive counterevidence closure is tracked separately and is
            # NOT a prerequisite, because the argument layer preserves BOTH.
            # If an opposite direct basis is found later, it adds the opposite
            # direction instead of invalidating this already-grounded fact.
        ...
        if positive_basis and not own_discovery_complete and purpose in {
            "POSITIVE_EXISTENCE_PROOF",
            "EXPLICIT_ADVERSE_PROOF",
        }:
