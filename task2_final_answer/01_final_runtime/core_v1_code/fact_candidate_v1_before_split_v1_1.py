...
def _explicit_positive_state(text: str) -> bool:
    t = _norm(text)
    patterns = (
        r"\ball gaps\b[^.]{0,120}\bsealed\b",
        r"\bgaps?\b[^.]{0,80}\b(?:sealed|closed|filled)\b",
...
def _polarity(quote: str, typed: dict) -> str:
    ...
    positive = bool(
        ...
        or signals & {"POSITIVE", "MIXED"}
        or _explicit_positive_state(quote)
    )

    if adverse and not actual and _instructional_adverse(quote):
        adverse = False

    explicit_adverse_state = bool(re.search(
        r"\b(?:worn|cracked|broken|damaged|delaminat(?:ed|ion)|"
        r"hole(?:s)?|repeated .* activity|delayed .* follow[- ]?up)\b",
        _norm(quote),
    ))

    if (
        _explicit_positive_state(quote)
        and "ADVERSE_OPERATIONAL_FINDING" not in natures
...
def build_fact_candidates(evidence_id: str, text: str) -> list[dict]:
    text = str(text or "")
    full = classify_evidence_nature(text)

    if not full.get("requires_subspan_fact_split", False):
        start = len(text) - len(text.lstrip())
        end = len(text.rstrip())
        if end <= start:
            return []
        return [_one(evidence_id, text, start, end, text[start:end])]

    pieces = []
    for start, end, quote in _segments(text):
        typed = classify_evidence_nature(quote)
        if typed.get("requires_subspan_fact_split", False):
            pieces.extend(_semicolon_segments(text, start, end))
        else:
            pieces.append((start, end, quote))

    facts = [_one(evidence_id, text, s, e, q) for s, e, q in pieces if q.strip()]
    return [f for f in facts if f["grounding_valid"]]
