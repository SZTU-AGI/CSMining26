...
def discover_cp_ids(contract_dir: Path) -> list[str]:
    """Discover only canonical CP<number>.json files.

    Historical/backup artifacts such as CP12_before_ruleset_relation.json are
    intentionally excluded even when they contain a valid-looking contract.
    """
    rows = []
    paths = [
        path
        for path in contract_dir.glob("CP*.json")
        if re.fullmatch(r"CP\d+", path.stem)
    ]
    paths.sort(key=lambda p: int(p.stem[2:]))
...
def runtime_preflight() -> dict:
    """Fail early if argument_core/helper wiring is stale."""
    helper_path = Path(argument_support.__file__).resolve()
    helper_sha256 = hashlib.sha256(helper_path.read_bytes()).hexdigest()

    helper_source = inspect.getsource(argument_support.evaluate_argument_template)
    core_source = inspect.getsource(argument.evaluate_benchmark_statement)

    helper_has_four_value_fix = (
        "support = all(bool(state.pair[0]) for state in facet_states)" in helper_source
        and "attack = any(bool(state.pair[1]) for state in facet_states)" in helper_source
    )
    core_uses_helper = (
        "multi_atom_argument_support_v1" in core_source
        and "evaluate_argument_template" in core_source
    )

    print("Runtime preflight:")
    print("  argument_core:", Path(argument.__file__).resolve())
    print("  argument helper:", helper_path)
    print("  helper sha256:", helper_sha256)
    print("  helper four-valued facet fix:", helper_has_four_value_fix)
    print("  argument_core delegates to helper:", core_uses_helper)

    if not helper_has_four_value_fix:
        raise RuntimeError(
            "STALE_MULTI_ATOM_ARGUMENT_HELPER: "
            "multi_atom_argument_support_v1.py does not contain the "
            "ALL-support / ANY-attack four-valued atom projection."
        )
    if not core_uses_helper:
        raise RuntimeError(
            "STALE_ARGUMENT_CORE_INSTALL: "
            "argument_core_v1.evaluate_benchmark_statement does not delegate "
            "to multi_atom_argument_support_v1.evaluate_argument_template."
        )

    return {
        "argument_core_path": str(Path(argument.__file__).resolve()),
        "argument_helper_path": str(helper_path),
        "argument_helper_sha256": helper_sha256,
        "helper_four_valued_fix": helper_has_four_value_fix,
        "argument_core_delegates_to_helper": core_uses_helper,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=Path(
            "results_v2/"
            "multi_atom_contract_audit_v1_3.json"
        ),
    )
    ...
    result = {
        "schema":
            "freca-multi-atom-contract-audit-v1-3",
        "all_pass": True,
        "contracts": rows,
