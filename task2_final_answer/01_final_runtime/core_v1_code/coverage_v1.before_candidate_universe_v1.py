...
def candidate_disposition(
    *,
    ...
    for candidate in candidates:
        ...
        # Some candidate identities are excluded before FactCandidate/model
        # alignment.  If the candidate trace has an explicit deterministic use
        # decision, it is still disposed; otherwise it is unassessed.
        if use:
            assessed.append(
                evidence_id
            )
...
def evaluate_need_coverage(
    *,
    ...
    if (
        ...
        limiting_factors.append(
            "MODEL_CONTEXT_CAP_REACHED"
        )

    if (
        candidate_limit > 0
        and not candidate_universe_preserved
    ):
        limiting_factors.append(
            "RANKED_PREFIX_LIMITED_BY_CANDIDATE_LIMIT"
        )
    ...
    if disposition[
        ...
        limiting_factors.append(
            "SELECTED_CANDIDATES_UNASSESSED"
        )
    ...
    # A conditional item is preserved as an identity gap signal.  This module
    # does not claim it is decisive enough, by itself, to make the entire
    # RequirementCoverage INCOMPLETE_IDENTITY.
    ...
    # Current Core has no formal parse-gap ledger wired into this artifact.
    parse_gap_ids = list(
        trace.get(
            "parse_gap_ids",
            [],
    ...
    # Status precedence follows structural inability to claim population
    # coverage.  Detailed limiting_factors preserve every reason.
    if parse_gap_ids:
        status = (
            "INCOMPLETE_PARSE"
        )
        ...
        # A future complete trace reaches here.
    ...
    report = {
        ...
        "policy_notes": [
            ...
            "MMR/ranking order does not establish coverage",
