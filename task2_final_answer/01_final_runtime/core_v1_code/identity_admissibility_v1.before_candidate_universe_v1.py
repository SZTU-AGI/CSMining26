...
def apply_identity_gate_to_traces(
    ...
    """Annotate every retrieval candidate with Layer-5-like identity use metadata.

    Nothing is deleted from the retrieval ledger.  D7 alignment may later skip
    EXCLUDE_SUBSTANTIVE candidates, while CONDITIONAL candidates may still be
    semantically inspected but cannot stand as decisive proof in this pilot.
    """

    out = copy.deepcopy(traces)
    source_relations = identity_report.get("source_relations") or {}

    for trace in out:
        direct = conditional = excluded = 0
        for candidate in trace.get("candidates", []):
            source_id = _candidate_source_id(candidate)
            source = source_relations.get(
                source_id,
                {
                    "relation_to_case": "CASE_ASSOCIATED_OWNER_UNKNOWN",
                    "status": "CONDITIONAL",
                },
            )
            relation = _chunk_owner_relation(
                str(candidate.get("text") or ""),
                str(source.get("relation_to_case") or "UNKNOWN"),
                identity_report,
            )
            decision, decisive_ok, reason = _use_decision(relation, identity_report)

            candidate["source_id"] = source_id
            candidate["identity_relation_to_case"] = relation
            candidate["identity_use_decision"] = decision
            candidate["identity_decisive_proof_eligible"] = decisive_ok
            candidate["identity_reason_code"] = reason

            if decision == "ADMIT_DIRECT":
                direct += 1
            elif decision == "EXCLUDE_SUBSTANTIVE":
                excluded += 1
            else:
                conditional += 1

        trace["identity_gate_summary"] = {
            "admit_direct": direct,
            "admit_conditional": conditional,
            "exclude_substantive": excluded,
        }
