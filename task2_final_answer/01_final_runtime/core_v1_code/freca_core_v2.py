...
def resolve_relative_subsection_refs(
    unit: dict,
    unit_map: dict[str, dict],
) -> list[str]:
    """
    Minimal extraction from full Layer-1 / Layer-2
    relative-reference closure.
    ...
    This is deterministic and contains no CP-specific map.
    """
    ...
    # Section citation = part before first "("
    #
    # 4-7A(2) -> 4-7A
    section_citation = (
        citation.split(
    ...
    for selected_id in selected_ids:
        ...
        if parent_id:
            ...
            if parent:
                ...
                # --------------------------------------------
                # Direct siblings / enum companions.
                # --------------------------------------------
                ...
                # --------------------------------------------
                # Deterministic relative-reference closure.
                #
                # Example:
                #
                #   4-7A(2):
                ...
                #   4-7A(1)
                #
                # Then expose the referenced subsection's
                # direct enum children as candidates.
                #
                # They are NOT automatically selected:
                # all rescued units still pass RelationDecision
                # and LegalBasisLink.
                # --------------------------------------------

                reference_targets = (
                    resolve_relative_subsection_refs(
                        parent,
                        unit_map,
                    )
                )
...
# ============================================================
# RuleSetRelation — minimal L2-C
#
# Extracted from full Layer-2 D2.26 / D2.46.
#
# IMPORTANT:
# LegalBasisLink answers:
#   "can this rule legally ground this CP?"
#
# RuleSetRelation answers:
#   "how do multiple grounded rules relate to the SAME
#    benchmark criterion?"
#
# This prevents:
#   multiple legal bases -> automatic ALL.
# ============================================================
...
RULE_SET_RELATION_SYSTEM = r"""
You are the RuleSetRelation reviewer in a closed-source legal
rule compiler.

You are NOT a compliance classifier.
You must not construct the final AST.
You must not output 1, 0, N/A, PASS or FAIL.

You receive:

1. one official FRECA checking-point criterion;
2. PolicyUnits that have already passed CandidateRelation and
   LegalBasisLink;
3. exact legal grounding information.

Your task is ONLY to determine how these grounded legal norms
relate to THIS benchmark criterion.

Allowed relations:

CUMULATIVE

    Every member represents a distinct requirement that the
    supplied benchmark criterion itself requires cumulatively.

    Use this ONLY when the benchmark CP structure and/or explicit
    legal connective structure establishes that failure of each
    member is independently material to satisfaction of this CP.

    Same topic is not enough.
    Multiple legal duties are not enough.
    Multiple legal bases are not enough.

ALTERNATIVE

    The members represent alternative ways to satisfy the same
    benchmark requirement and the supplied source structure
    establishes the alternative relationship.

SPECIALIZES

    One member is a narrower or more specific legal expression of
    another member.

    General + specific versions must not automatically become two
    independent required benchmark leaves.

SUPPORTS_SAME_CRITERION

    Multiple members provide legal grounding, operationalisation,
    corroboration, or different expressions of the SAME benchmark
    requirement.

    They may jointly support one proposition, but must not be
    converted into separate REQUIRED_ALL_OF leaves merely because
    they are all legally relevant.

CONTEXT_ONLY

    The relationship is contextual and should not determine
    benchmark aggregation.

CONTESTED

    Two or more materially plausible source-grounded
    interpretations exist.

UNRESOLVED

    The supplied text does not establish the relationship.

CRITICAL ANTI-OVERATOMIZATION RULE:

Finer-grained Rules obligations do NOT automatically become
finer-grained benchmark requirements.

The official CP defines the scoring object.

For example, if several legal provisions all explain or
operationalise one broad CP phrase, prefer
SUPPORTS_SAME_CRITERION unless the supplied CP/legal connective
structure proves that each must independently fail the whole CP.

CUMULATIVE and ALTERNATIVE require explicit connector evidence.

Connector evidence may come from:
- the official CP; or
- a supplied Rules PolicyUnit.

Every quoted phrase must be exact.

Groups MAY overlap.

For example:
- a parent and child may form a SPECIALIZES group;
- several maintenance provisions may form a
  SUPPORTS_SAME_CRITERION group;
- representative design and maintenance provisions may separately
  participate in a CUMULATIVE group if the CP explicitly requires
  both dimensions.
...

{
  "event_relation_groups": [
    {
      ...
      "relation": "SUPPORTS_SAME_CRITERION",
  ...
  "rationale_summary": "brief summary"
}

For CUMULATIVE or ALTERNATIVE:
source_evidence MUST contain at least one item with role=CONNECTOR.

For RULES source evidence:
candidate_id must identify the supplied PolicyUnit containing the
quote.
...
def make_rule_set_relation_prompt(
    cp: dict,
    candidates: list[dict],
    ledger: list[dict],
) -> str:
...
Classify the material relationships among these grounded norms.

Do not build the final contract.

Return JSON only.
"""


def validate_rule_set_relation(
    raw: dict,
    cp: dict,
    candidates: list[dict],
    ledger: list[dict],
    ...
    for group in groups:
        ...
        if (
            criterion_quote
            and quote_match_mode(
                criterion_quote,
                cp["criterion"],
            ) is None
        ):
            raise ValueError(
                f"{group_id}: criterion "
    ...
    # If there are multiple eligible rules,
    # the relation layer must not silently
    # ignore one of them.
    if len(
        eligible_ids
    ) > 1:
...
def review_rule_set_relations(
    ...
    raw = core.deepseek_json(
        model=
            core.CONTRACT_MODEL,
        system_prompt=
            RULE_SET_RELATION_SYSTEM,
        user_prompt=
            make_rule_set_relation_prompt(
                cp,
                candidates,
                ledger,
            ),
...
The validated RULE-SET RELATION artifact is authoritative for
...
SUPPORTS_SAME_CRITERION
    ...
    Members may jointly ground ONE benchmark proposition.
    They must NOT become separate REQUIRED ALL children solely
    because they are separate Rules provisions.

SPECIALIZES

    General and specific members must not become duplicate
    independent required leaves merely because both are valid
    legal bases.

CUMULATIVE

    Members may become separate required branches ONLY where the
    validated group contains grounded CONNECTOR evidence.

ALTERNATIVE

    Members may become ANY branches only where the validated group
    contains grounded CONNECTOR evidence.

CONTEXT_ONLY

    Must not create a decisive satisfaction leaf.

CONTESTED / UNRESOLVED

    Must not be silently resolved by this compiler.

ANTI-OVERATOMIZATION:

Legal detail is not the same as benchmark leaf count.

If several provisions operationalise or support one broad CP
requirement, prefer one proposition with multiple legal bases
rather than several ALL children.

A child of the top-level satisfaction ALL means:
"If this child is false, the whole CP can fail."

Do not create such a child unless the official CP / validated
RuleSetRelation explicitly supports that failure propagation.
...
def compile_cp_v2(
    ...
    # --------------------------------------------------------
    # RuleSetRelation — L2-C
    # --------------------------------------------------------

    print(
        "\nRuleSetRelation review:"
    )

    rule_set_relation = (
        review_rule_set_relations(
            cp,
            candidates,
            ledger,
        )
