...
for c in "${cases[@]}"; do
  echo "Launching smoke $c"
  (PYTHONPATH="$CORE" conda run --no-capture-output -n freca-core python "$V61/code/fast_final_adjudicator_v6_6.py" --run-root "$RUN" --case "$c") >"$RUN/logs/smoke-$c.log" 2>&1 & pids+=("$!")
done
fail=0; for p in "${pids[@]}"; do wait "$p" || fail=1; done
if ((fail)); then echo 'Smoke worker failed'; tail -n 80 "$RUN"/logs/smoke-*.log; exit 1; fi
