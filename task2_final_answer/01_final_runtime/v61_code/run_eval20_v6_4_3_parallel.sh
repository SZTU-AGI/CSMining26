...
CASE_FILE="${V643_CASE_FILE:-$V61/code/eval20_cases_v6_3.txt}"
BASE_OUT="${V643_EVAL_ROOT:-$V61/results/eval20_4cps_v6_4_3_initial}"
...
for shard_file in "$BASE_OUT/_case_shards"/worker-*.txt; do
  ...
  (
    PYTHONPATH="$CORE" conda run --no-capture-output -n freca-core \
      python "$CORE/production_runner_v6_4_3.py" \
...
echo "Next: bash $V61/code/replay_eval20_v6_4_3.sh"
