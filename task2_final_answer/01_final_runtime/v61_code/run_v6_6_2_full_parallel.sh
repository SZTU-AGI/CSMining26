...
mkdir -p "$RUN/logs" "$RUN/_shards_v662"
python - "$RUN/_shards_v662" "$WORKERS" <<'PY'
...
pids=(); names=()
for sf in "$RUN/_shards_v662"/worker-*.txt; do
 name="$(basename "$sf" .txt)"; args=()
 while IFS= read -r c; do [[ -n "$c" ]] && args+=(--case "$c"); done < "$sf"
 echo "Launching V6.6.2 $name (${#args[@]} cases)"
 (PYTHONPATH="$CORE:$V61/code" conda run --no-capture-output -n freca-core python "$V61/code/fast_final_adjudicator_v6_6_2.py" --run-root "$RUN" "${args[@]}") >"$RUN/logs/v662-$name.log" 2>&1 &
 pids+=("$!"); names+=("$name")
done
fail=0
for i in "${!pids[@]}"; do if wait "${pids[$i]}"; then echo "[${names[$i]}] COMPLETE"; else echo "[${names[$i]}] FAILED"; tail -n 100 "$RUN/logs/v662-${names[$i]}.log"; fail=1; fi; done
...
PYTHONPATH="$CORE:$V61/code" conda run --no-capture-output -n freca-core python "$V61/code/finalize_v6_6_2_submission.py" --run-root "$RUN"
