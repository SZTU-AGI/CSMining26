    ...
    if evid["requires_subspan_fact_split"]:
        ...
        if model_relation == "SUPPORT":
            ...
            if NATURE_REGISTRATION_DEFECT in natures:
                decision = DIRECT
                reason = "REGISTERED_OPERATION_SCOPE_ATTACKED_BY_EXPLICIT_SCOPE_DEFECT"
        ...
        if model_relation == "SUPPORT":
            if NATURE_EQUIPMENT_CONDITION in natures and _equipment_subject_match(requirement, exact_quote):
                decision = DIRECT
                reason = "EQUIPMENT_FITNESS_SUPPORTED_BY_CURRENT_CONDITION_EVIDENCE"
            ...
            if NATURE_CURRENT_DEFECT in natures and re.search(
                r"\b(station|trap|bait|equipment|device)\b", _norm(exact_quote)
            ):
                decision = DIRECT
        ...
        if model_relation == "SUPPORT":
            ...
            } and mode not in {MODALITY_REQUIRED, MODALITY_PLANNED} and _risk_control_subject_match(exact_quote):
                decision = DIRECT
                reason = "RISK_CONTROL_STATE_SUPPORTED_BY_ACTUAL_OUTCOME_EVIDENCE"
            elif natures & {
                NATURE_REVIEW_FINDING,
                NATURE_OBSERVATION_RECORD,
            } and evid["assertion_mode"].get("actual_signal_present") and _risk_control_subject_match(exact_quote):
                decision = DIRECT
            ...
            if NATURE_CONTROL_ABSENCE in natures and _risk_control_subject_match(exact_quote):
                decision = DIRECT
                reason = "RISK_CONTROL_STATE_ATTACKED_BY_EXPLICIT_CONTROL_ABSENCE"
            elif natures & {
                NATURE_ADVERSE_OPERATIONAL_FINDING,
                NATURE_CURRENT_DEFECT,
            } and _risk_control_subject_match(exact_quote):
                decision = DIRECT
        ...
        if model_relation == "SUPPORT":
            if mode == MODALITY_ACTUAL and natures & {
                ...
                NATURE_OBSERVATION_RECORD,
                NATURE_REVIEW_FINDING,
            } and _activity_anchor_match(requirement, exact_quote):
                decision = DIRECT
