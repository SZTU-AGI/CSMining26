...
VERSION='V6.6.2_HYBRID_FORCED_NA_GATE_1'; CP=[f'CP{i}' for i in range(1,42)]; ALLOWED={'0','1','N/A'}
...
def main():
 ...
 ap.add_argument('--template',type=Path,default=Path('/home/MeggieYu/freca/Task2/submission_template.xlsx')); ap.add_argument('--output',type=Path,default=None)
 a=ap.parse_args(); out=a.output or a.run_root/'submission_v6_6_2_hybrid_final.xlsx'
 ...
 for case in cases:
  ...
  for cp in CP:
   ...
   audit.append({'case_uid':uid,'cp_id':cp,'verdict':v,'reason':row.get('reason'),'evidence_ids':row.get('evidence_ids') or [],'na_verified':row.get('na_verified',False),'na_gate_evidence':row.get('na_gate_non_applicability_evidence') or []})
 wb=load_workbook(a.template); ws=wb.active; header=[ws.cell(1,c).value for c in range(1,43)]
 ...
 audit_path=a.run_root/'submission_v6_6_2_hybrid_final_audit.json'; save({'version':VERSION,'counts':dict(counts),'output':str(out),'rows':audit},audit_path)
 print('FINAL SUBMISSION READY'); print('Output:',out); print('Counts:',dict(counts)); print('Audit:',audit_path)
